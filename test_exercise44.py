...
import string
import datetime
from exercise44 import create_appointments_table, insert_one_appointment, get_appointments
...
def test_retrieve_all_appointments(db_connection, monkeypatch, three_appointments):
    ...
    assert 'US Holiday: Independence Day' in output[0]
    assert 'US Holiday' not in output[1]
    assert 'US Holiday' not in output[2]
